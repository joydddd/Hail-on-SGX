...
Client::Client(std::string clientname, std::string hostname, std::string server_hostname, int listen_port, int server_port) 
    : clientname(clientname), hostname(hostname), server_hostname(server_hostname), listen_port(listen_port), server_port(server_port) {
    init();
}
...
void Client::init() {
    send_msg("REGISTER", hostname + " " + std::to_string(listen_port));
    blocks_sent = 0;
}
    ...
    switch (mtype) {
        ...
        {   
            ...
            for (int i = 0; i < std::stoi(msg); ++i) {
                response = std::to_string(blocks_sent++) + " BLOCK " + std::to_string(i);
                send_msg(msg_type, response);
            }
            break;
        }
        default:
...
void Client::send_msg(const std::string& msg_type, const std::string& msg) {
    std::string message = clientname + " " + std::to_string(msg.length()) + " " + msg_type +"\n" + msg;
    send_message(server_hostname.c_str(), server_port, message.c_str());
}